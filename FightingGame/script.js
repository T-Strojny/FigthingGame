...
class Player {
  ...
  strike (player, enemy, attackDmg) {
    ...
    //  Return a message of 'player name attacks enemy name for damageAmount'
        return `${player.name} attacks ${enemy.name} for ${damageAmount} damge!`;
  }
  ...
  declareWinner(isOver, p1, p2) {
    ...
    // If isOver is true AND p1 health is <= 0 then update message variable  to 'p1 WINS!'
        if (isOver == true && p1.health <= 0) {
            winnerMessage = `${p2.name} WINS !`
        } else if (isOver == true && p2.health) {
            winnerMessage = `${p1.name} WINS !`
        }
...
let ronaldo = new Player('Ronaldo', 100, 10);
let messi = new Player('Messi', 100, 10);
...
document.addEventListener('keydown', function(e) {
  ...
  if (p1.health > 0 && e.key == "a" && game.isOver == false) {
    p1.heal(p1);
...
document.addEventListener('keydown', function(e) {
  ...
  if (p2.health > 0 && e.key == "l" && game.isOver == false) {
    p2.heal(p2);
